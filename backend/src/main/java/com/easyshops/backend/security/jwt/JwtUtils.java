...
public class JwtUtils {
  ...
  @Value("${auth.token.expirationInMils}") // expiration 7 days in milliseconds
  ...
  public String generateTokenForUser(Authentication authentication) {
    ...
    return Jwts.builder().setSubject(userPrincipal.getEmail())
               .claim("id", userPrincipal.getId()).claim("roles", roles)
  ...
  /**
   * Validates the JWT token for expiration, signature, and format.
   */
